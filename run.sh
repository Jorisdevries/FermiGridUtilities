...
source create_dataset.sh "$user_name" "$project_name" "$larsoft_release_name" "$tarball_path" "$max_files" "$events_per_file" "$prestage" "$file_prefix" "$resumbit" "$resumbit_cycles" "$is_data" "$fcl_file_name" 
